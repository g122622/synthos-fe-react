...
export const getGroupDetails = async (): Promise<
...
export const getChatMessagesByGroupId = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/chat-messages-by-group-id?${params}`
  );
  return response.json();
};
...
export const getAIDigestResultByTopicId = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/ai-digest-result-by-topic-id?${params}`
  );
  return response.json();
};
...
export const getAIDigestResultsBySessionId = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/ai-digest-results-by-session-id?${params}`
  );
  return response.json();
};
...
export const isSessionSummarized = async (
  ...
  const params = new URLSearchParams({ sessionId });
  const response = await fetch(
    `${API_BASE_URL}/api/is-session-summarized?${params}`
  );
  return response.json();
};
...
export const getQQAvatar = async (
  qqNumber: string
): Promise<ApiResponse<QQAvatarResponse>> => {
